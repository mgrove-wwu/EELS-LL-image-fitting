...
class EELS_image_fit(object): 
    ...
    def load_model(self, mkey=None):
        ...
        if (mkey == None):
            ...
            if (mkey == 'exit' or mkey == 'cancel'):
                ...
                self.get_model()
                self.function_set = self.split(self.all_models[mkey], ('_'))[0]
                ...
                try:
                    ...
                    self.get_model()
                    self.function_set = self.split(self.all_models[mname], ('_'))[0]
                    self.Chisq        = self.Fit_Model.chisq
                    self.red_Chisq    = self.Fit_Model.red_chisq
    ...
    def save_evaluation(self,
        ...
        print('Writing goodness and - if possible - thickness images to disk...')
    ...
    def line_roi(self, 
        ...
        if (interactive == True):
            ...
            while (answer == False):
                plt.ion()
                line = self.line.interactive(param_map)
                plt.show()
                plt.draw()
                plt.pause(time)
    ...
    def line_variance(self, 
        ...
        for step in range(size):
            ...
            rotated_line = line_rot(param_map)
    ...
    def generate_linescan(self,
        ...
        line_var, line_var_fstd  = self.line_variance(param_map, line_scan)
    ...
    def plot_pshift(self,
        ...
        if (np.isfinite(np.array(std_gaus,dtype=float)).all() or np.isfinite(np.array(std_poly,dtype=float)).all):
    ...
    def estimate_parametershift(self,
        ...
        if (only_poly == False):
            ...
            print(substracted.axes_manager)
            ...
            if (force_positive == True):
                try:
                    ...
                    peaks_positive       = np.array([np.array([(position,std*0.1,position/2)], 
                                                    dtype=[('position', '<f8'), ('height', '<f8'), ('width', '<f8')])]
                                                   )
                    
                result = self.check_peaks(peaks_positive, max_pos, scale, offset, sensitivity)
                ...
                try:
                    ...
                    peaks_negative       = np.array([np.array([(position,-std*0.1,position/2)], 
                                                    dtype=[('position', '<f8'), ('height', '<f8'), ('width', '<f8')])]
                                                   )
                    
                result = self.check_peaks(peaks_negative, max_pos, scale, offset, sensitivity)
                ...
                try:
                    ...
                    peaks_positive       = np.array([np.array([(position,std*0.5,position/2)], 
                                                    dtype=[('position', '<f8'), ('height', '<f8'), ('width', '<f8')])]
                                                   )
                ...
                try:
                    ...
                    peaks_negative       = np.array([np.array([(position,-std*0.5,position/2)], 
                                                    dtype=[('position', '<f8'), ('height', '<f8'), ('width', '<f8')])]
                                                   )
                
                peaks_concatenate = np.concatenate((peaks_positive[0], peaks_negative[0]), axis=0)
                result = self.check_peaks(peaks_concatenate, max_pos, scale, offset, sensitivity)
